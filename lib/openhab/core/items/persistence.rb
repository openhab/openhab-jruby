  ...
  module Core
    module Items
      ...
      module Persistence
        ...
        #   @param [#to_zoned_date_time] timestamp The point in time until which to search
        #   @param [Symbol, String] service An optional persistence id instead of the default persistence service.
        #   @return [HistoricState, nil] The maximum value until `timestamp`,
        #     or nil if no future states could be found.
        #   @since openHAB 4.2
        ...
        #   @param [#to_zoned_date_time] timestamp The point in time until which to search
        #   @param [Symbol, String] service An optional persistence id instead of the default persistence service.
        #   @return [HistoricState, nil] The minimum value until `timestamp`,
        #     or nil if no future states could be found.
        #   @since openHAB 4.2
        ...
        #
        def persist(*args)
          service = args.pop || persistence_service

          # @deprecated OH 4.1 this whole if block can be removed in OH 4.2
        ...
        %i[previous_state next_state].each do |method|
          define_method(method) do |service = nil, skip_equal: false|
            service ||= persistence_service
            result = Actions::PersistenceExtensions.public_send(method, self, skip_equal, service&.to_s)
            wrap_result(result, quantify: true)
          end
        end
        ...
        def_persistence_method(:historic_state)
        ...
        if OpenHAB::Core.version >= OpenHAB::Core::V4_2
          def_persistence_method(:persisted_state)
        ...
        def wrap_result(result, quantify: false)
          ...
          when org.openhab.core.persistence.HistoricItem
            HistoricState.new(result, quantify ? quantify(result.state) : nil)
