  ...
  module Core
    ...
    module Proxy
      ...
      def initialize(target)
        @klass = KERNEL_CLASS.bind(self).call

        if target.is_a?(@klass::Type)
          super
          KERNEL_IVAR_SET.bind(self).call(:"@#{@klass::UID_METHOD}", target&.__send__(@klass::UID_METHOD))
        else
          # dummy items; we were just passed the item name
          super(nil)
          KERNEL_IVAR_SET.bind(self).call(:"@#{@klass::UID_METHOD}", target)
        end
      end
